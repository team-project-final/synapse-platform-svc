...
- notification 모듈 파일 목록 (DeviceTokenController, DeviceTokenService 등)
- TASK_platform.md: Done When 체크박스 미체크 / Status 줄은 Done 표기 (불일치 상태)
- **추가 확정**: 팀 회의 결정에 따라 Refresh Token은 사용자당 최대 5개 멀티 디바이스 세션 허용 (5개 초과 시 FIFO 만료)

## 현재 미결 사항

- Refresh Token 멀티 디바이스 지원 설계 적용 (V28 마이그레이션, Repository/Service/Controller 코드 수정 및 테스트 반영)
...
- 한 사용자 최대 5개 디바이스 (FCM)
- platform 값: `ios`, `android`, `web` (소문자)
- Refresh Token: 사용자당 최대 5개 active 토큰 허용 (5개 초과 시 FIFO 만료 적용)
- Refresh Token 원문 저장 금지 (token_hash SHA-256 저장)
